   ...
   for (unsigned int i = 0; i < mAVFormatContext->GetStreamsCount(); i++)
   {
      ...
      if (stream->IsAudio())
      {
         ...
         const sampleFormat preferredFormat =
            codecContextPtr->GetPreferredSneedacitySampleFormat();
         ...
         if (stream->GetDuration() > 0)
            ...
            duration = mAVFormatContext->GetDuration() / SNEEDACITY_AV_TIME_BASE;
              ...
              Tags *tags)
{
   ...
   for (auto &stream : mChannels)
   {
      ...
      if (streamStartTime != int64_t(SNEEDACITY_AV_NOPTS_VALUE) && streamStartTime > 0)
      {
      ...
      if (stream_delay > 0)
      {
         ...
         for (auto &channel : stream)
         {
            ...
            t->InsertSilence(0,double(stream_delay)/SNEEDACITY_AV_TIME_BASE);
         }
      }
   }
...
ProgressResult FFmpegImportFileHandle::WriteData(StreamContext *sc, const AVPacketWrapper* packet)
{
   ...
   if (
      packet->GetPresentationTimestamp() != SNEEDACITY_AV_NOPTS_VALUE &&
      mAVFormatContext->GetDuration() != SNEEDACITY_AV_NOPTS_VALUE)
   {
      ...
      mProgressLen =
         (mAVFormatContext->GetDuration() > 0 ?
             mAVFormatContext->GetDuration() / SNEEDACITY_AV_TIME_BASE :
             1);
   }
