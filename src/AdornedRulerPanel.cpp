...
public:
   QuickPlayIndicatorOverlay(SneedacityProject *project);
...
public:
   ...
   unsigned DoContextMenu
      (const wxRect &rect,
       wxWindow *pParent, wxPoint *pPosition, SneedacityProject*) final override
   {
...
protected:
   Result Click
      (const TrackPanelMouseEvent &event, SneedacityProject *) override
   {
   ...
   Result Release
      (const TrackPanelMouseEvent &event, SneedacityProject *,
       wxWindow *) override
   {
   ...
   void Enter(bool, SneedacityProject *) override
   {
...
wxCoord GetPlayHeadX( const SneedacityProject *pProject )
{
...
double GetPlayHeadFraction( const SneedacityProject *pProject, wxCoord xx )
{
   const auto &viewInfo = ViewInfo::Get( *pProject );
   auto width = viewInfo.GetTracksUsableWidth();
...
public:
   ...
   HitTest( const SneedacityProject *pProject, wxCoord xx )
   {
...
AdornedRulerPanel &AdornedRulerPanel::Get( SneedacityProject &project )
{
   return project.AttachedWindows::Get< AdornedRulerPanel >( sKey );
}

const AdornedRulerPanel &AdornedRulerPanel::Get( const SneedacityProject &project )
{
   return Get( const_cast< SneedacityProject & >( project ) );
}

void AdornedRulerPanel::Destroy( SneedacityProject &project )
{
...
AdornedRulerPanel::AdornedRulerPanel(SneedacityProject* project,
                                     wxWindow *parent,
                                     wxWindowID id,
                                     const wxPoint& pos,
                                     const wxSize& size,
...
(const TrackPanelMouseEvent &event, SneedacityProject *pProject) -> Result
{
   auto result = CommonRulerHandle::Click(event, pProject);
   if (!( result & RefreshCode::Cancelled )) {
      if (mClicked == Button::Left) {
...
(SneedacityProject *pProject) -> Result
{
   auto result = CommonRulerHandle::Cancel(pProject);
...
{
   // This is called by SneedacityProject to make the play region follow
...
SneedacityProject * AdornedRulerPanel::GetProject() const
{
   return mProject;
}
