...
ProjectRate &ProjectRate::Get( SneedacityProject &project )
{
   return project.AttachedObjects::Get< ProjectRate >( sKey );
}

const ProjectRate &ProjectRate::Get( const SneedacityProject &project )
{
   return Get( const_cast< SneedacityProject & >( project ) );
}

ProjectRate::ProjectRate(SneedacityProject &project)
{
   int intRate = 0;
