...
class SNEEDACITY_DLL_API ScreenshotCommand : public SneedacityCommand
{
public:
   enum kBackgrounds
   {
...
private:
   ...
   void CaptureEffects(const CommandContext & Context, SneedacityProject * pProject, const wxString &fileName );
   void CaptureScriptables(const CommandContext & Context, SneedacityProject * pProject, const wxString &fileName );
   void CapturePreferences(const CommandContext & Context, SneedacityProject * pProject, const wxString &fileName );
   bool Capture(
   ...
   wxRect GetTrackRect( SneedacityProject * pProj, TrackPanel * panel,int n);
   wxString WindowFileName(SneedacityProject * proj, wxTopLevelWindow *w);

public:
   static ScreenshotCommand * mpShooter;
   static void (*mIdleHandler)(wxIdleEvent& event);
   static void SetIdleHandler( SneedacityProject &project );
   static bool MayCapture( wxDialog * pDlg );

   void CaptureWindowOnIdle( const CommandContext & context, wxWindow * pWin );
   wxTopLevelWindow *GetFrontWindow(SneedacityProject *project);
};
