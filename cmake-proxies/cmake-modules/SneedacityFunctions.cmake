...
# Taken from wxWidgets and modified for Sneedacity
#
...
# the manual. We could add that property to the Sneedacity target and
# CMake would add the required nodes to the VS project. And when the
# Sneedacity target is built, the NuGet packages would get automatically
# downloaded. This also means that the locale and manual targets
# must be dependent on the Sneedacity target so the packages would get
...
#
# We want the Sneedacity target to be the startup project to provide
# easier debugging. But, if we do that, the ALL_BUILD target is no
# longer "in control" and any dependents of the Sneedacity target would
...
# not be available during Sneedacity debugging because the Nyquist runtime
# would not be copied into the destination folder alonside the Sneedacity
# executable.
#
# To remedy this conundrum, we simply download the NuGet packages
# ourselves and make the Sneedacity target dependent on the targets
# mentioned above. This ensures that the dest folder is populated
# and laid out like Sneedacity expects.
#
...
function( sneedacity_append_common_compiler_options var use_pch )
   if( NOT use_pch )
      list( APPEND ${var}
         PRIVATE
   ...
   list( APPEND ${var}
         -DSNEEDACITY_VERSION=${SNEEDACITY_VERSION}
         -DSNEEDACITY_RELEASE=${SNEEDACITY_RELEASE}
         -DSNEEDACITY_REVISION=${SNEEDACITY_REVISION}
         -DSNEEDACITY_MODLEVEL=${SNEEDACITY_MODLEVEL}

         # Version string for visual display
         -DSNEEDACITY_VERSION_STRING=L"${SNEEDACITY_VERSION}.${SNEEDACITY_RELEASE}.${SNEEDACITY_REVISION}${SNEEDACITY_SUFFIX}"

         # This value is used in the resource compiler for Windows
         -DSNEEDACITY_FILE_VERSION=L"${SNEEDACITY_VERSION},${SNEEDACITY_RELEASE},${SNEEDACITY_REVISION},${SNEEDACITY_MODLEVEL}"
   ...
   # Definitions controlled by the SNEEDACITY_BUILD_LEVEL switch
...
function( sneedacity_module_fn NAME SOURCES IMPORT_TARGETS
...
#   list( TRANSFORM SOURCES PREPEND "${CMAKE_CURRENT_SOURCE_DIR}/" )
   ...
   set( OPTIONS )
   sneedacity_append_common_compiler_options( OPTIONS NO )
   ...
   if (NOT LIBTYPE STREQUAL "MODULE")
      list( APPEND GRAPH_EDGES "\"Sneedacity\" -> \"${TARGET}\"" )
   endif ()
...
# may be given too.
macro( sneedacity_module NAME SOURCES IMPORT_TARGETS
...
# More defines, and more target libraries (maybe generator expressions)
# may be given too.
macro( sneedacity_library NAME SOURCES IMPORT_TARGETS
   ADDITIONAL_DEFINES ADDITIONAL_LIBRARIES )
   ...
   list( APPEND SNEEDACITY_LIBRARIES "${NAME}" )
   set( SNEEDACITY_LIBRARIES "${SNEEDACITY_LIBRARIES}" PARENT_SCOPE )
endmacro()

# A special macro for header only libraries

macro( sneedacity_header_only_library NAME SOURCES IMPORT_TARGETS
   ADDITIONAL_DEFINES )
   # ditto comment in the previous macro
...
#
#     symbol   suffix for the "USE_<symbol>" variable that the Sneedacity
